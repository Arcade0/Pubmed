...
import http.client
from Bio import Entrez, Medline  # 一般是通过BioPython的Bio.Entrez模块访问Entrez
...
def query(
    ...
    for sp in sp_l:
        ...
        # 创建记录已完成搜索的物种的文件
        with open("%s/%s" % (input_path, sp.replace(".txt", "_done.txt")),
                  "a") as df:
        ...
        for spec in spec_l:
            ...
            mk_dir("%s/%s/%s" % (output_path, sp.replace(
                ".txt", ""), spec_file.replace(" ", "_")))  # 保存格式 类/种名
            ...
            for term in term_l:
                ...
                term_file = term.replace("\"", "")
                output_name = "%s/%s/%s/%s.txt" % (
                    output_path, sp.replace(".txt", ""), spec_file.replace(
                        " ", "_"), term_file.replace(" ", "_"))
                ...
                if len(idlist) > 0:
                    ...
                    # 是否索取文献更详细的信息
                    if fur_query is True:
                        
                        dic = {}

                        for start in range(0, total, step):

                            print("Download record %i to %i" %
                                  (start + 1, int(start + step)))
                            handle_1 = Entrez.efetch(db="pubmed",
                                                     retstart=start,
                                                     rettype="medline",
                                                     retmode="text",
                                                     retmax=step,
                        ...
                        with open(output_name.replace("txt", "json"),
                                  'w') as f:
