        ...
        for spec in spec_l:
            ...
            for file in file_l:
                ...
                for paper in data_l:
                    ...
                    # 检查这个paper是否已经对比完成
                    anno_folder = "%s/%s/%s/%s" % (output_path, sp, spec,
                                                   file.replace(".json", ""))
                    chek = os.path.exists(
                        "%s/%s.htmll" %
                        (anno_folder, paper["id"]))  # wrong for test
                    if not chek:
                        ...
                        sub = [
                            "TITLE:", "ABSTRACT", "BACKGROUND:", "METHOD:",
                            "METHODS:", "FINGDING", "FINDINGS:", "RESULT",
                        ...
                        if len(key_l) > 0:  # 筛选3
                            ...
                            full_gene_l = title_gene_l + [
                                i['text'] for i in ab_anno
                                if i['infons']['type'] == 'Gene'
                            ]
                            full_che_l = title_species_l + [
                                i['text'] for i in ab_anno
                                if i['infons']['type'] == 'Chemical'
                            ]

                            full_text = "https://pubmed.ncbi.nlm.nih.gov/%s/" % (
                                paper["id"])
                            ...
                            full_ls = [[
                                key,
                                replace_html_tag(full, spec_key[key])
                            ] for key in spec_key.keys()]
                            full_ls.append([
                                "Disease",
                                replace_html_tag(full, full_disease_l)
                            ])
                            full_ls.append([
                                "Species",
                                replace_html_tag(full, full_species_l)
                            ])
